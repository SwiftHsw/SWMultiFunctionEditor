...
static const void *WZBTextViewisFirst = &WZBTextViewisFirst;
...
-(void)setIsBegin:(BOOL)isBegin{
    //    self.isFirst= isBegin;
    objc_setAssociatedObject(self, WZBTextViewisFirst, [NSString stringWithFormat:@"%lld",(long long)isBegin], OBJC_ASSOCIATION_COPY_NONATOMIC);
}
//世文加的
-(BOOL)isBegin{
    return  [objc_getAssociatedObject(self, WZBTextViewisFirst) boolValue];;
}
-(BOOL)wzb_isFirst{
    return [objc_getAssociatedObject(self, WZBTextViewisFirst) boolValue];
...
//        frame.origin.y = frame.origin.y-5;
        self.wzb_placeholderView.frame = frame;
//        self.wzb_placeholderView.backgroundColor = [UIColor greenColor];
