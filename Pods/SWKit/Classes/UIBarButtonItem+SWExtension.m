...
+ (instancetype)itemWithTitle:(NSString *)title ImageName:(NSString *)imageName highImageName:(NSString *)highImageName target:(id)target action:(SEL)action
{
    ...
    UIColor *color = ([title isEqualToString:@"保存"] || [title isEqualToString:@"浏览"] || [title isEqualToString:@"返回"] || [title isEqualToString:@"完成"])  ? [UIColor blackColor] : [UIColor whiteColor];
